...
- **ソースレベル意味復元** - AST `deep` モードを既定で有効化し、圧縮されたローカル名を `params`、`requestData`、`response`、`event`、`app` など監査しやすい名前へ書き戻し
- **追跡可能な AST 書き戻し** - `ast_rename_map.json`、`ast_rename_diff.md`、`ast_rename.patch` を生成し、`semantic -ast-rollback=true` 用に書き戻し前ソースを保持
- **API セマンティックビュー** - `api_map`、API 呼び出しチェーン、監査用疑似コードを生成し、Burp 生リクエストをソース API に関連付け可能
...
#### 新機能
- AST レベルの変数 / 関数名リネームを追加し、既定の `-ast-rename=deep` で high / medium 信頼度のローカル変数、関数引数、ローカル関数名を書き戻し
- `off/report/safe/deep` の AST モードを追加し、実行時に現在の戦略、書き換え範囲、保護対象、ロールバック方法を CLI に表示
- `.gwxapkg/ast_rename_map.json`、`.gwxapkg/ast_rename_diff.md`、`.gwxapkg/ast_rename.patch`、`.gwxapkg/pre_ast_sources` を追加し、追跡可能な書き戻しと復元をサポート
- API 呼び出しチェーン `.gwxapkg/api_call_chain.json` / `.gwxapkg/api_call_chain.md` を追加
- AST API 監査用疑似コード `.gwxapkg/api_pseudo.md` と `.gwxapkg/pseudo_api/*.js` を追加
- Burp の生リクエストをファイルまたは stdin から読み取り、ソース API に関連付ける `api-link` コマンドを追加

#### 改良
- `scan`、`all`、既定の解包コマンド、`semantic -dir` が `-ast-rename`、`-ast-diff`、`-ast-patch` に対応
- AST 書き戻しでは `exports.xxx`、オブジェクトキー、文字列、コメント、WXML handler、`wx/uni/getApp/require/module/exports` などの公開 / グローバル識別子を明示的に保護
- `semantic_module_map.json` に AST 統計、API 呼び出しチェーンのパス、API 疑似コードのパスを追加し、監査追跡性を向上

#### 検証
- AST モード、信頼度ゲート、diff / patch / rollback、API 呼び出しチェーン、疑似コード、Burp リクエスト関連付けの回帰テストを追加
- `go test ./...` と `go build ./...` で検証済み
